...
/**
 * An implementation of the LocalizableAction abstract class which cuts the selected text when performed.
 * 
 * @author fabjanvucina
 */
public class CutAction extends LocalizableAction {
	 ...
	 * @param frame
	 */
	public CutAction(ILocalizationProvider lp, JNotepadPP frame) {
		super("cut", "cutAccel", "cutMnem", "cutDescript", lp);
		this.frame = frame;
	...
	public void actionPerformed(ActionEvent e) {
		...
		frame.editTextEditor(new ITextEditor() {
			...
			public void edit(int len, int offset, Action pasteAction, String clipboard)
				...
				//delete
				frame.getEditor().getDocument().remove(offset, len);
			}
		});
	}
}
